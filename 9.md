...
Swift 2 에서 protocol 들을 확장하고 기본 구현들을 제공하는 방법이 소개되었습니다. Swift 3 에서는 연산자 준수(conformance)가 개선되었고, 표준 라이브러이에서 이러한 개선(연산자 호환 향상)들이 새로운 숫자 protocol 들에 사용되었습니다.
...
**CustomStringConvertible** protocol 에 호환되도록 **Bird** protocol 선언을 수정하세요.
```
protocol Bird: CustomStringConvertible {
```
**CustomStringConvertible** 에 호환된다는 것은 String 처럼 동작하기 위해서 **description** property를 가질 필요가 있다는 것을 의미합니다.
...
그리고 모든 collection wrapper들이 이 protocol에 호환되도록 정의했습니다. 이것은 **Array** 에 대해 map을 호출하는 것을 **ReversedRanddomAccessCollection** 에
대해 호출하는 것 만큼이나 쉽고 별 차이점을 느낄 수 없게 만듭니다. 이 중요한 디자인 패턴을 잠시 빌려올 겁니다.

## 경주에 나갑시다.

이제까지 여러분은 여러가지 **Bird** 호환 타입을 정의했습니다. 이제 playground 끝에 완전히 다른 어떤 것을 추가하세요.
```
class Motorcycle {
  init(name: String) {
    self.name = name
...
```
이 코드에서 protocol **Racer** 를 처음으로 정의합니다. 그리고 나서 모든 다른 타입들과 호환되게 만듭니다. **Motorcycle** 같은 몇몇 타입들은 간단하게 호환됩니다.
**UnladenSwallow** 같은 다른 것들은 좀 더 많은 로직이 필요합니다. 마침내, 여러개의 **Racer** 호환 타입들을 가지게 되었습니다.

모든 호환 타입을 가지고 racer 들의 배열을 만듭니다.
...
```
이 코드들은 조금 무서워 보일지도 모릅니다. 그럼 잘게 나눠 봅시다. RacerType 은 이 함수에 대한 generic 타입이고, Swift 표준 라이브러리의 **Sequence** protocol에 호환하는 어떤 type이든 될 수 있습니다. **where** 절은 sequence의 element 타입이 **Racer** protocol 에 호환되어야 함을 나타냅니다. **Sequence** 타입들은 **Iterator** 라고 불리는 관계된 타입을 가집니다. **Iterator** 타입은 **Elment** 타입들을 통해서 순환(loop)할 수 있습니다. 실제 메소드 본체(body)는 이전과 것과 거의 같습니다.
...
높은 점수가 누구인지 이야기 할 수 있도록 점수들이 비교가능하기를 원합니다. Swift 3 이전에는, 이런 protocol들과 호환되는 global operator 함수들을 추가 해야 했었습니다.
...
연산자 호환에 대한 논리적 근거는 Swift evolution proposal( https://github.com/apple/swift-evolution/blob/master/proposals/0091-improving-operators-in-protocols.md )에서 찾을 수 있습니다.
