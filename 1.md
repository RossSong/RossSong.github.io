이글은  "BY JURE ZOVE IN SWIFT 26 COMMENTS, A practical MVVM example in Swift – Part 1"에 대한 번역입니다.
...
# A practical MVVM example in Swift – Part 1(bold) #
한동안 꽤 괜찮았던 MVC 패턴이라는 것이 있었습니다.
...
View와 Controller가 결합되어 있는 형태라서 UI관련 부분과 로직 처리 부분이 혼재되어 거대해지기 쉽다.
...
MVC가 어떤 모습이어야 하는지를 보여주는 아름다운 구조가 여기 있다.
...
(View에서 대한 많은 처리를 Model에서 하게 되고, dependency가 높아진다.)
...
심지어 첫번째 그림보다는 좀 더 명확해 보입니다. 그렇지 않은가요? Controller는 이제 Model은 신경쓰지 않고,
View Model리 제공하는 것을 보여주는 역할을 하게 되었습니다. View도 마찬가지입니다. 이제 UILable, UITextField 그리고
UIImageView들은 View Model이 제공하는 것만 단지 보여줍니다.
...
Car와 CarViewModel 둘다 non-optional 파라미터들만 를 받는 initializer를 구셩했기 때문에
...
```
**모든 것이 완벽합니다! 그런데 왜 테스트를 하나구요?(bold)**
...
온전히 동작하는 프로젝는 깃헙에서 당신을 기다리고 있습니다.(https://github.com/jurezove/mvvm-swift/tree/v1.0)
