...
우리는 이 발생한(emitted) 이벤트들을 단지 비동기로(asynchronously) 획득할 수 있습니다. 값이 발생(emitted) 때 실행되는 함수, 오류(error)가 발생할(emitted) 때 실행되는 다른 함수, "completed" 신호가 발생할(emitted) 때 실행되는 다른 함수들을 정의 함으로써 이벤트를 획득할 수 있습니다.
