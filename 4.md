...
## 3가지 제안을 stream 으로 모델링하기

지금까지, 우리는 responseStream의 subscribe()에서 발생하는 그리는 단계의 제안 UI element 를 다뤄왔습니다.
이제는 refresh button과 관련해서 문제가 있가 있습니다.:여러분이 'refresh'를 클릭하지 마자, 현재의 3가지 제안이 지워지지 않습니다.
응답(response)이 도착한 이후에 새로운 제안이 들어 옵니다. 하지만 UI를 멋지게 만들기 위해서, 우리는 refresh 가 click 될때 현재 제안들을 모두 지울 필요가 있습니다.
...
아니요, 여러분 너무 빠르지는 않습니다. 이것은 좋지 않습니다. 우리는 제안 DOM 요소들에 영향을 주는(하나는 responseStream.subscribe()) ***두 개의*** subscriber들을 가지고 있습니다.
