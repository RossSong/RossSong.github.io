...
결과는 리스트의 stream입니다. 그 stream은 우리가 map()을 적용시켜서 각각의 리스트를 리스트의 길이와 매칭되는 숫자로 (map)한 것입니다. 마지막으로 우리는 숫자 1들을 filter(x >= 2)를 이용해서 무시합니다. 의도된 stream을 생산하기 위한 3가지 연산들이 그겁니다. 그럼 다음, 우리가 원하는 대로 반응하도록 subscribe("listen")할 수 있습니다.
