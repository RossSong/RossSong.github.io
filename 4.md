...
stream은 시간 순서대로 진행중인 이벤트들의 순서배열(sequence)입니다. stream은 세가지 다른 것들을 발생(emit)할 수 있습니다.: (어떤 타입의) 값, 오류(error),
"완료됨" 신호("completed" signal). "completed" 신호가 발생했다고 가정해보세요. 예를 들어, 버튼을 포함한 현재 윈도우 혹은 view가 닫혔을때(closed)를
가정해 보세요.

우리는 이 발생한(emitted) 이벤트들을 단지 비동기로(asynchronously) 획득할 수 있습니다. 값이 발생될(emitted) 때 실행되는 함수, 오류(error)가 발생할(emitted) 때 실행되는 다른 함수, "completed" 신호가 발생할(emitted) 때 실행되는 다른 함수들을 정의 함으로써 이벤트를 획득할 수 있습니다.
...
: 각각의 발생한(emitted) 값은 다른 stream에 대한 포인터입니다. 우리 예제에서는, 각각의 요청(request) URL 은 관계된 응답(response) 포함하는 promise stream 에 대한 포인터로 사상(map)됩니다.
...
가지("branch") stream들에 발생한(emitted) 모든 것을 "trunck" stream 에 발생시킴(emitting)으로써 mainstream 을 넓게 펴는("flattens") map의 한 버전입니다.
...
refresh button 이 click될 때마다, 요청(request) stream은 새로운 URL을 발생시켜야(emit) 하고 그렇게 해서 새로운 응답(response)를 받을 수 있다.
