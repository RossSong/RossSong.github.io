...
그 곳이 "functional"의 마술이 효과를 발휘하는 곳입니다. stream은 다른 stream의 입력으로 사용될 수 있습니다. 심지어 여러개의 stream들이 다른 stream의 입력으로 사용될 수 있습니다. 여러분은 두개의 stream들을 합칠수(merge)도 있습니다. 여러분은 관심있는 이벤트만을 가지는 stream을 얻기 위해서 stream을 거를수(filter) 있습니다. 여러분은 한 stream으로 부터의 data 값을 다른 새로운 stream으로 위치시킬(map)수도 있습니다.
